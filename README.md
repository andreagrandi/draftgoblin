...
See [docs/pick-scoring.md](docs/pick-scoring.md) for the initial 0–100 scoring model and integer tie-display decision. The Textual watch view uses `q` to quit, `c` to toggle secondary columns, and `s` to cycle score/ALSA/mana-value sorting. See [docs/deck-builder.md](docs/deck-builder.md) for deck-builder constraints, mana-base defaults, relaxation order, and the inert v1 `--allow-splash` flag.
