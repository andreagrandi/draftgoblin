...
See [docs/pick-scoring.md](docs/pick-scoring.md) for the 17Lands WR/grade display, the 0–100 Draftgoblin scoring model, and integer tie-display decision. The Textual watch view uses `q` to quit, `c` to toggle secondary columns, `s` to cycle ranking between 17Lands WR (default), DG score, ALSA, and mana value, `b` to open the build view, `t` to open the post-draft backtest report, and `m` to toggle opt-in Mana font icons. See [docs/deck-builder.md](docs/deck-builder.md) for deck-builder constraints, 17Lands structure targets, mana-base defaults, relaxation order, and `--allow-splash`.
