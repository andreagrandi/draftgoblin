...
Compare raw 17Lands WR recommendations against Draftgoblin scores.
"""
...
from draftgoblin.ranking import rank_scored_cards, ranking_label
...
class PickBenchmarkComparison:
    """Direct DG-vs-17L rank comparison.
...
def format_pick_benchmark_report(report: PickBenchmarkReport) -> str:
    ...
    lines = [
        "Draftgoblin trophy pick benchmark",
        ...
        (
            "Default ranking decision: DG Score is the default because "
...
def _format_comparison(*, comparison: PickBenchmarkComparison) -> str:
    ...
    if total == 0:
        return "DG vs 17L actual-pick rank: no comparable picks."

    return (
        "DG vs 17L actual-pick rank: "
...
def _format_heuristic_note(*, report: PickBenchmarkReport) -> str:
    ...
    if late_off_color_misses > 0:
        return (
            ...
            f"{late_off_color_misses} DG Score misses were building/locked "
    ...
    if neutral_prior_misses > 0:
        return (
            "Non-ML heuristic candidate from misses: "
            f"{neutral_prior_misses} DG Score misses used neutral-prior data; "
            "test ALSA and maindeck-rate weighting before ML work (#37)."
        )

    return (
        "Non-ML heuristic candidate from misses: review DG Score misses by "
